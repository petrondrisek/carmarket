...
const images = car.images.map((image: string) => asset(image));
</script>
...
<template>
    ...
    <div v-else>
        <Error v-if="car.is_sold" message="This offer is no longer available." />
        ...
        <CarHero :car="car">
            <template #gallery>
                <button 
                    ...
                    class="text-4xl cursor-pointer absolute bottom-32 right-4 z-10" 
                ...
                <SwiperGallery v-if="images.length > 0" :images="images" />
            </template>
        ...
        <SwiperFullscreen v-if="images.length > 0" :show="showFullscreen" @close="showFullscreen = false" :images="images" />
        </div>
</template>
